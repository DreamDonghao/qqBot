...
namespace LittleMeowBot {
    ...
    public:
        ...
        /// @param groupId 群号
        /// @return 矢期记忆文本（每行一条）
        [[nodiscard]] std::string getShortTermMemory(const uint64_t groupId) const{
            return Database::instance().getLongTermMemory(groupId);
        }

        /// @brief 更新群的短期记忆
        /// @param groupId 群号
        /// @param memoryText 记忆文本（每行一条）
        void updateShortTermMemory(const uint64_t groupId, const std::string& memoryText) const{
...
=== 最近对话 ===
        ...
        /// @brief 合并新旧记忆（去重、归纳）
        /// @param existingMemory 现有记忆文本
        /// @param newMemory 新记忆条目
        /// @return 合并后的记忆文本
        drogon::Task<std::string> mergeMemories(
            const std::string& existingMemory,
            const std::string& newMemory
        ) const{
...
新增记忆：
        ...
        /// @param groupId 群号
        /// @param chatRecords 聊天记录文本
        drogon::Task<void> appendAndMergeMemory(const uint64_t groupId, const std::string& chatRecords) const{
            // 1. 生成新记忆
        ...
        /// @brief 从短期记忆迁移到长期记忆库
        /// @param groupId 群号
        /// @param shortTermMemory 当前短期记忆
        /// @return 迁移后的短期记忆（保留最多shortTermMemoryMax条）
        drogon::Task<std::string> migrateToLongTermMemory(
            const uint64_t groupId,
            const std::string& shortTermMemory
        ) const{
...
短期记忆：
        ...
        /// @param maxLines 最大条数
        /// @return 截断后的记忆
        [[nodiscard]] std::string trimToMaxLines(const std::string& memory, const int maxLines) const{
